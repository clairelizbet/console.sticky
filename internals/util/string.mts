...
export function isCommonAlphaChar(char: string): boolean {
...
export function convertCamelCaseToKebabCase(camelCased: string): string {
  let kebabCased: string = ''
  let lastCharIndex: number | undefined
  const camelCasedLength = camelCased.length

  for (
    let nextCharIndex = 0;
    nextCharIndex <= camelCasedLength;
    nextCharIndex++
  ) {
