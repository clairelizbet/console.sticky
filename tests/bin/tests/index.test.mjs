...
// Main module to test
import { addConsoleSticky, addStyledConsoleSticky, getAllConsoleStickies, removeAllConsoleStickies, } from '../index.mjs';
import { StickyPreset } from '../presets.mjs';
// Internals used for test setup and teardown
import { attachConsoleListenerPassthrus, detachConsoleListenerPassthrus, } from '../internals/sticky/glue.mjs';
...
describe('addConsoleSticky', () => {
    ...
    it('returns a key symbol and logs serializable objects as strings', () => {
        ...
        expect(logCallCount()).toBe(prevLogCallCount + 1);
        expect(mockedConsoleLog).toHaveBeenLastCalledWith(JSON.stringify(serializableObject));
    });
    ...
    it('returns a key symbol and logs with preset styles', () => {
        expect(typeof addStyledConsoleSticky(plainText, StickyPreset.ImportantNotice)).toBe('symbol');
        expect(logCallCount()).toBe(prevLogCallCount + 1);
        expect(mockedConsoleLog).toHaveBeenLastCalledWith(`%c${plainText}`, 'color:white;background-color:blue;padding:20px;font-size:24px;');
    });
    it('returns a key symbol and logs with preset styles and format specifier', () => {
        const message = `%c${plainText}`;
        expect(typeof addStyledConsoleSticky(message, StickyPreset.ImportantNotice)).toBe('symbol');
        expect(logCallCount()).toBe(prevLogCallCount + 1);
        expect(mockedConsoleLog).toHaveBeenLastCalledWith(message, 'color:white;background-color:blue;padding:20px;font-size:24px;');
    });
    it('returns a key symbol and logs with multiple presets', () => {
        const message = `%c${plainText} %c${plainText}`;
        expect(typeof addStyledConsoleSticky(message, StickyPreset.ImportantNotice, StickyPreset.ImportantWarning)).toBe('symbol');
        expect(logCallCount()).toBe(prevLogCallCount + 1);
        expect(mockedConsoleLog).toHaveBeenLastCalledWith(message, 'color:white;background-color:blue;padding:20px;font-size:24px;', 'color:white;background-color:orange;padding:20px;font-size:24px;');
    });
    it('returns a key symbol and logs with custom styles', () => {
        expect(typeof addStyledConsoleSticky(plainText, {
            color: 'white',
            fontSize: '24px',
            backgroundColor: 'red',
            padding: '20px',
        ...
        expect(logCallCount()).toBe(prevLogCallCount + 1);
        expect(mockedConsoleLog).toHaveBeenLastCalledWith(`%c${plainText}`, 'color:white;font-size:24px;background-color:red;padding:20px;');
    });
